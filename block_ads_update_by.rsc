# ================================================
# 1: MikroTik Advanced AD Blocker Pro - BY List Updater
# ================================================
...
#
# DESCRIPTION:
# Automatic updater for Belarusian advertisement domain lists.
# ================================================

:local remoteURL "https://raw.githubusercontent.com/paulmann/1-mikrotik-advanced-ad-blocker-pro/main/block_ads_by.rsc"
:local tempFile "block_ads_by_temp.rsc"
:local currentVersion "5.0.0"
:local markerComment "BY_Ad_Domain_v5"
:local addressListName "AD_Blocker_Domains"

:log info "Starting Belarusian domain list update check..."

# Create backup
:local backupName "by_list_backup_$[/system clock get date]"
/system backup save name="$backupName" dont-encrypt=yes
:log info "Created backup: $backupName"

# Download latest list
:log info "Downloading latest Belarusian domain list..."
...
:if ([:len [/file find name="$tempFile"]] > 0) do={
    ...
    :if ($newVersion != "" && $newVersion != $currentVersion) do={
        ...
        :log info "Belarusian list updated: v$currentVersion -> v$newVersion (+$addedDomains)"
        
        :put "======================================================"
        :put "BELARUSIAN DOMAIN LIST UPDATE SUCCESSFUL"
        :put "======================================================"
        ...
        :log info "Belarusian domain list is already up-to-date (v$currentVersion)"
        /file remove $tempFile
        
        :put "======================================================"
        :put "BELARUSIAN DOMAIN LIST STATUS"
        :put "======================================================"
    ...
    :log error "Failed to download updated Belarusian domain list"
    :put "ERROR: Unable to download list from GitHub"
}

# Create scheduler
:if ([/system scheduler find name="adblocker_update_by"] = "") do={
    /system scheduler add name="adblocker_update_by" interval=7d start-time="04:45:00" \
        on-event=":delay 30s; /import file-name=block_ads_update_by.rsc" \
        comment="Weekly update for Belarusian AD Blocker domains"
